...
var (
	...
	// ErrConflict indicates that an optimistic generation or conditional
	// storage operation failed because the durable object changed.
	ErrConflict = errors.New("storage object generation conflict")
	...
	// ErrSourceEqualsDestination indicates that a source and destination are
	// the same object.
...
// Unwrap exposes both the stable error category and the underlying cause.
//
// This permits callers to use errors.Is with either value.
...
// Wrap creates a storage operation error with a stable category and an
